...
// ResolveConfig reads PORT/DBPATH/NEXTCLOUD_URL from the injected env lookup, so values
// supplied by a .env file (godotenv populates the process env) or by real environment
// variables take effect. These cases pin that precedence and the numeric ID1_AUTH_PORT
// guard.
func TestResolveConfig(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			port, dbpath, ncURL := ResolveConfig(getenv)
			if port != tt.wantPort {
